...
完整的 Shadowrocket 自托管规则包。56 个唯一规则来源（4 个自定义来源＋52 个外部来源）按照原始顺序分别保存为 56 个名称清晰的远程 `.list` 文件，例如 `ChatGPT.list`、`Google.list`、`OKX.list` 和 `Binance.list`，避免把约 17 万条规则直接展开进主配置。
...
- `rules/`：Shadowrocket 实际下载的 56 个独立规则文件。
